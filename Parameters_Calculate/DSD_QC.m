...
%Description:
%DSD characters(quality control)
% History:
% 2020-11-23 by zhangtc
...
load('E:\CODE\OTTparsivel2_mat\data_parameters\DSD_parameters.mat','central_dia_qc');
load('E:\CODE\OTTparsivel2_mat\data_parameters\DSD_parameters.mat','central_speed');
load('E:\CODE\OTTparsivel2_mat\data_parameters\DSD_parameters.mat','dia_bandwidth_qc');
for fnum = 3:length(file_day)
    fname = [file_root,file_day(fnum).name];
    
    Mputu = zeros(32,32,1440);
    rainflag = zeros(1440,1);
    typeflag = zeros(1440,1)-1;
    ...
    for ifile =1:length(ftxts)
        ...
        if sum(sum(temp_putu)) > 10
            ...
            for di = 3:23
                for vj = 1:32
                    ND(mTime_putu,di) = ND(mTime_putu,di) + temp_putu(vj,di)./(0.0054.*60 .* central_speed(vj).*dia_bandwidth_qc(di));
                    RR(mTime_putu) = RR(mTime_putu) + 6 * pi * (1e-4) .* temp_putu(vj,di) .* central_dia_qc(di)^3 ./ (0.0054.*60);
                    LWC(mTime_putu) = LWC(mTime_putu) + pi/6 * (1e-3) .* temp_putu(vj,di) .* central_dia_qc(di)^3 ./ (0.0054.*60.*central_speed(vj));
                end
                M2(mTime_putu) = M2(mTime_putu) + ND(mTime_putu,di) .*central_dia_qc(di)^2 .* dia_bandwidth_qc(di);
                M3(mTime_putu) = M3(mTime_putu) + ND(mTime_putu,di) .*central_dia_qc(di)^3 .* dia_bandwidth_qc(di);
                M4(mTime_putu) = M4(mTime_putu) + ND(mTime_putu,di) .*central_dia_qc(di)^4 .* dia_bandwidth_qc(di);
                M6(mTime_putu) = M6(mTime_putu) + ND(mTime_putu,di) .*central_dia_qc(di)^6 .* dia_bandwidth_qc(di);
            end
            ...
            temp = 0;
            for di = 3:23
                temp = temp + (central_dia_qc(di)-Dm(mTime_putu))^2 .* ...
                    ND(mTime_putu,di) .*central_dia_qc(di)^3 .* dia_bandwidth_qc(di);
            end
...
%                     typeflag(temp_loc(num)) = 1;%convective
        ...
        hdf5writedata(savename, '/Dm', Dm, ...
            'dataAttr', ...
            struct('Units', '', 'long_name', 'mass-weighted mean diameter'));
